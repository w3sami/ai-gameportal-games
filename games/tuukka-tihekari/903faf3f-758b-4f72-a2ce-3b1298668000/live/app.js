(() => {
  ...
  const state = { manifest:null, tracks:[], done:0, bytes:0, active:0, retries:0, failed:[], paused:false, started:0, current:-1, filtered:[], shuffle:false, repeat:false, objectUrls:new Map(), catalog:null, page:0, loadingPage:false };
  ...
  async function install(){
    const configRes=await fetch('manifest.json',{cache:'no-store'}),c=await configRes.json();
    const indexRes=await fetch(c.catalogIndex||'music/catalog-index.json',{cache:'no-store'}),catalog=state.catalog=await indexRes.json();
    if(!catalog.pageCount)throw Error('Music catalog is unavailable');await loadNextPage();if(!state.tracks.length)throw Error('Music catalog is empty');unlock();
  }
  async function loadNextPage(){if(state.loadingPage||!state.catalog||state.page>=state.catalog.pageCount)return;state.loadingPage=true;try{while(state.page<state.catalog.pageCount){const page=state.page+1,url=state.catalog.pagePattern.replace('{page}',page),data=await (await fetch(url,{cache:'no-store'})).json(),added=data.tracks||[];state.page=page;if(added.length){state.tracks.push(...added);break}}if(!els.player.hidden){populateFilters();render();showCredits()}}finally{state.loadingPage=false}}
  ...
  els['track-list'].addEventListener('scroll',()=>{if(els['track-list'].scrollTop+els['track-list'].clientHeight>els['track-list'].scrollHeight-180)loadNextPage()});
