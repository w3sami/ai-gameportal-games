(() => {
  ...
  async function install(){
    ...
    const manifestRes=await fetch(c.libraryManifest||'music/library-manifest.json',{cache:'no-store'}),m=state.manifest=await manifestRes.json();
    if(!m.tracks?.length || m.trackCount!==m.tracks.length)throw Error('Music catalog is unavailable');
    state.tracks=m.tracks; unlock();
  }
