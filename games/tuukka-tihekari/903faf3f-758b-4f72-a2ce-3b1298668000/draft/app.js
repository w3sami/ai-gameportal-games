(() => {
  ...
  async function loadNextPage(){if(state.loadingPage||!state.catalog||state.page>=state.catalog.pageCount)return;state.loadingPage=true;try{const page=state.page+1,url=state.catalog.pagePattern.replace('{page}',page),data=await (await fetch(url,{cache:'no-store'})).json();state.tracks.push(...(data.tracks||[]));state.page=page;if(!els.player.hidden){populateFilters();render();showCredits()}}finally{state.loadingPage=false}}
