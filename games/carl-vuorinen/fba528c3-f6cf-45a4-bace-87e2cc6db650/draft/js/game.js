...
const Snd = (() => {
  ...
  function init(){
    ...
    // Crash kit: white noise, a baked bed of debris clicks, a dark ~2 s cave reverb, and a compressor bus so stacked
    ...
    for (let ch=0;ch<2;ch++){ const r = ir.getChannelData(ch); let lp = 0;
      for (let i=0;i<r.length;i++){ const t = i/sr; lp += (Math.random()*2-1 - lp)*0.18; r[i] = lp*Math.exp(-t/0.38)*(t < 0.012 ? t/0.012 : 1); } }
  ...
  function boom(){
    ...
    voice(crackle, v(), [bq('highpass', 1500*v()), bq('lowpass', 7000, 2000, 1.2)],       [0.05,  0.5, 0.0,   0.5 ]);   // debris
  }
  return {
