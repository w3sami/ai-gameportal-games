 ...
 */
export async function submit({ score, name, board = "default", data = {} } = {}) {
  let result;
  try {
    ...
    result = await call(boardUrl(board), {
      ...
      body: JSON.stringify({ score, name: who, data }),
    });
  } catch (err) {
 ...
 * their rank first.
 */
export async function top({ board = "default", limit = 10, offset, around } = {}) {
  let result;
  try {
 ...
 *
 * Attributes: `board`, `limit`, `around`, `theme`, `refresh` in seconds for a
...
class LeaderboardPanel extends HTMLElement {
  static observedAttributes = [
    "board", "limit", "around", "theme", "refresh", "empty-text", "error-text",
  ];
  ...
  async reload() {
    ...
    const page = await top({ board, limit, around });
    if (page.reason) {
