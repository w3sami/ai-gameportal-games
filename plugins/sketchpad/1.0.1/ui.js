...
.sketchpad .list { display: grid; gap: 4px; max-height: 196px; overflow: auto; }
.sketchpad .list li {
...
export function createPanel(core) {
  ...
  function handle(ctx, it, size) {
    ...
    if (on) {
      ...
      ctx.setLineDash([]);
      ctx.globalAlpha = 1;
      tag(ctx, x + 11, y - 9, `${it.label || it.id}  ${round(x)}, ${round(y)}`, PICK, true, size);
    }
    ctx.restore();
  }
  ...
  function pick(p) {
    ...
    for (const it of core.itemsOf()) {
      const b = boxOf(it);
      if (b && hit({ x: p.x, y: p.y, w: 1, h: 1 }, b)) return it;
    }
    return null;
  }
  ...
  function onDown(e) {
    ...
    if (tool === "move") {
      ...
      if (sel) {
        snapshot();
        drag = { it: sel, ox: sel.obj.x - p.x, oy: sel.obj.y - p.y };
      }
      build();
      return;
    }
