...
const TEXT = {
  en: {
    ...
    add: "place",
    drop: "remove placed",
    placed: "{n} placed, not in the source at all",
    ...
    add: "lisää",
    drop: "poista lisätty",
    placed: "{n} lisättyä, ei vielä lähdekoodissa lainkaan",
...
.sketchpad .xy { display: flex; gap: 6px; margin-bottom: 11px; }
.sketchpad .xy label { flex: 1; display: flex; align-items: center; gap: 6px; margin: 0; color: #8a97be; }
.sketchpad .xy input {
  width: 100%; min-width: 0; font: inherit; padding: 6px 8px; border-radius: 8px;
  color: #e9edff; background: rgba(255,255,255,.06);
  border: 1px solid rgba(120,160,255,.28);
  touch-action: auto; -webkit-user-select: text; user-select: text;
}
.sketchpad .xy input:focus { outline: none; border-color: rgba(111,227,255,.6); }
.sketchpad .xy input:disabled { opacity: .4; }
...
export function createPanel(core) {
  ...
  /* One step back, covering both the painting and the positions — otherwise
     ...
     pixel of it. */
  function snapshot() {
    undoSnap = JSON.stringify({
      shapes,
      pos: core.movesOf(core.sceneOf()),
      adds: core.addsOf(core.sceneOf()),
    });
  }
  function undo() {
    if (!undoSnap) return;
    ...
    shapes = back.shapes;
    sel = null;
    core.applyAdds(back.adds);
    for (const it of core.itemsOf()) {
      const d = core.defOf(it.id);
      if (!d) continue;
      const m = back.pos.find((p) => p.id === it.id);
      /* Jokainen oletuksen avain erikseen, ja siirrosta vain se mitä siinä on.
         Siirto kantaa vain muuttuneet luvut, joten `m.to.x` on usein
         undefined — se kirjoitettuna teki oliosta NaN:in ensimmäisellä
         raahauksella, ja NaN-koordinaatti ei piirry mihinkään. */
      for (const k of Object.keys(d)) {
        const v = m && isNum(m.to[k]) ? m.to[k] : d[k];
        if (isNum(v)) it.obj[k] = v;
      }
    }
    core.remember();
    keep();
  ...
  function queueCheck() {
    ...
    checkT = setTimeout(() => {
      recheck();
      /* Ei rakenneta uusiksi kesken kirjoittamisen: se veisi kohdistuksen
         keskeltä lukua. Punaiset kehykset päivittyvät silti, koska piirto lukee
         varoitukset joka ruudulla. */
  ...
  /** Kaikki muu kuin paikka: skaala, peilaus, kierto ja knob. */
  function extras(it) {
    const o = it.obj;
    const t = core.tfOf(it);
    ...
    if (t.scale || t.flip) {
      ...
      if (sx !== 1 || sy !== 1) {
        let one = `${Math.abs(sx) === Math.abs(sy) ? Math.abs(sx) : `${sx}, ${sy}`}×`;
        if (sx < 0) one += ' ⇔';
        if (sy < 0) one += ' ⇕';
        bits.push(one);
      }
    }
    if (t.rot && rotOf(it)) bits.push(`${core.tidy('rot', rotOf(it))}°`);
    if (k && isNum(o[k.key])) bits.push(`${k.key} ${core.tidy(k.key, o[k.key])}`);
    return bits.join(' · ');
  }

  /** Olion luvut yhtenä rivinä kankaalle, siinä järjestyksessä kuin luetaan. */
  function reading(it) {
    const e = extras(it);
    return `${round(it.obj.x)}, ${round(it.obj.y)}` + (e ? ` · ${e}` : '');
  }
  ...
  /* Kelvoton luku pysäytetään kirjoitushetkellä eikä vasta tallennuksessa.
     Yksikin NaN olion x:ssä tarkoittaa että olio katoaa ruudulta — ja koska se
     leviää seuraavaan laskutoimitukseen, sitä ei saa takaisin raahaamalla.
     Vika oli kumoamisessa, mutta vahinko oli tämän kokoinen, joten tämä jää
     myös sitä varten mitä ei vielä tiedetä. */
  function put(it, k, v) {
    if (isNum(v)) {
      it.obj[k] = v;
      return;
    }
    if (isNum(it.obj[k])) return;              // pidetään entinen
    const d = core.defOf(it.id);
    it.obj[k] = d && isNum(d[k]) ? d[k] : 0;
  }
  ...
  function onMove(e) {
    ...
    if (drag.it) {
      ...
      if (drag.mode === "rot") {
        ...
        put(it, "rot", core.tidy("rot", p.free ? deg : Math.round(deg / ROT_STEP) * ROT_STEP));
        ...
        put(it, "sx", core.tidy("sx", Math.sign(isNum(it.obj.sx) ? it.obj.sx : 1) * k));
        put(it, "sy", core.tidy("sy", Math.sign(isNum(it.obj.sy) ? it.obj.sy : 1) * k));
      } else {
        put(it, "x", grid(p.x + drag.ox, p.free));
        put(it, "y", grid(p.y + drag.oy, p.free));
      }
      call(o.onMove);
      coords();
      return;
  ...
  function onUp(e) {
    ...
    if (tiny) {
      ...
      sel = under || null;                     // a click on nothing clears it
      if (under) label = under.label;
      build();
      return;
    }
    snapshot();
  ...
  function onKey(e) {
    ...
    put(sel, "x", sel.obj.x + d[0] * step);
    put(sel, "y", sel.obj.y + d[1] * step);
    core.remember();
    keep();
    queueCheck();
    coords();
  ...
  let xyIn = null;
  function coords() {
    const on = sel && sel.obj && core.itemsOf().includes(sel) ? sel : null;
    if (coordEl) coordEl.textContent = on ? extras(on) || "—" : "—";
    if (!xyIn || !on) return;
    /* Kentän arvoa ei kirjoiteta yli sen päälle jota juuri kirjoitetaan. */
    for (const [i, k] of ["x", "y"].entries()) {
      if (document.activeElement === xyIn[i]) continue;
      xyIn[i].value = isNum(on.obj[k]) ? String(round(on.obj[k])) : "";
    }
  }
  ...
  function moveSection() {
    ...
    for (const it of items) {
      ...
      li.append(
        dot,
        el("span", "n", (it.added ? "+ " : off ? "• " : "") + (it.label || it.id)),
    ...
    box.append(row);
    reveal(ul, selRow);
    /* Valmiit assetit. Paneeli ei osaa piirtää uutta lajia — propsi on
       piirtofunktio eli koodia — mutta kenttä voi kertoa mitä se osaa piirtää,
       ja silloin niitä saa panna lisää. Uusi ilmestyy kentän keskelle ja jää
       valituksi, jotta sen voi heti raahata tai kirjoittaa paikalleen. */
    const kinds = core.spawnList();
    if (kinds.length) {
      const row = el("div", "row");
      const seg = el("div", "seg");
      for (const k of kinds) {
        const b = button(null, "", () => {
          snapshot();
          const size = core.size;
          const it = core.addOne(k.key, { x: Math.round(size.w / 2), y: Math.round(size.h / 2) });
          if (it) {
            sel = it;
            keep();
            recheck();
        ...
        b.append(document.createTextNode(k.label || k.key));
        seg.append(b);
      }
      row.append(el("label", null, T.add), seg);
      box.append(row);
    }
    ...
    /* Paikka myös kirjoitettavana. Raahaus on nopea ja epätarkka, ja tämä on
       se toinen puoli: tarkka luku silloin kun se tiedetään — ja ainoa tie
       takaisin jos olio on jostain syystä joutunut mahdottomaan paikkaan. */
    const xy = el("div", "xy");
    xyIn = ["x", "y"].map((k) => {
      const lab = el("label");
      const inp = el("input");
      ...
      inp.addEventListener("input", () => {
        ...
        if (!isNum(v)) return;                 // kesken kirjoitettu luku
        put(on, k, v);
        call(o.onMove);
        core.remember();
        queueCheck();
      ...
      lab.append(document.createTextNode(k), inp);
      xy.append(lab);
      return inp;
    });
    box.append(xy);
  ...
  function build() {
    ...
    segRow(
      ...
      labels,
      (l) => l.key === label,
      /* The label always applies to whatever is selected and stays on as the
         kind for the next shape — the way colour works in a drawing tool. To
         pick the next kind without touching the last one, click empty canvas
         first: that clears the selection. */
      (l) => {
        label = l.key;
        if (isShape(sel)) {
          snapshot();
    ...
    const ad = core.addsOf(core.sceneOf());
    if (ad.length) box.append(el("p", "hint", T.placed.replace("{n}", String(ad.length))));
    ...
    const canDrop = tool === "move" && sel && sel.added;
    const delBtn = button("ghost", canDrop ? T.drop : T.del, () => {
      snapshot();
      if (canDrop) {
        core.removeOne(sel);
        sel = null;
    ...
    delBtn.disabled = !canDrop && !isShape(sel);
  ...
  /** Read the saved sketch again and take its word if it is newer. */
  function resync() {
    return core.syncFromGame().then((held) => {
      if (held === null) return;
      savedBody = held;
      shapes = listOf(core.paint[core.sceneOf()]).slice();
      sel = null;
  ...
  return { draw, destroy, refresh, resync };
}
